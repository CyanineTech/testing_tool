...
1. 大多数问题都是“故障已经发生后，再回头查原因”。
2. 排障时应优先固定故障时间、重启时间、上一个 boot 或对应日志目录。
3. 当前状态只能作为补充证据，不能默认当作历史故障根因。
...
5. 如果是主机或机器人重启后的复盘问题，优先写“先找重启点/时间点，再回看历史日志”的步骤。
...
1. 先判断问题属于哪一层。
2. 先判断它是“实时状态问题”还是“历史故障复盘问题”。
3. 直接把新文档补到对应目录下。
4. 如果是新的大类，再先补目录说明或入口文档。
5. 如果会被现有路由命中，再同步更新知识索引和候选文档策略。
...
1. 先把文档放到正确目录。
2. 先写清楚故障发生时间、重启点、关键历史日志入口。
3. 再判断它是否属于高频场景。
4. 如果是高频场景，再补 `index.py` 的显式匹配。
5. 如果是旧路径迁移，再补 `loader.py` alias。
6. 最后更新对应目录 `README.md` 或总入口说明。
