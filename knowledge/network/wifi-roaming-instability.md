...
- 这类问题通常是历史掉线问题，优先记录掉线时间、位置、路线和恢复时间。
- 如果用户描述是“刚刚掉线”“跑到某个位置就掉线”“今天下午在电梯口掉线”，应优先按该时间窗口回看历史网络证据。
- 不要只因为当前静止状态下 `ping` 正常，就排除当时移动过程中的漫游问题。
...
- [customer-site-multi-floor-or-elevator-network-special-cases.md](customer-site-multi-floor-or-elevator-network-special-cases.md)
